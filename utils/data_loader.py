...
def dir_label_matrix(num_clients, alpha, prior):
    return np.random.dirichlet(alpha=alpha * prior, size=num_clients)
...
def get_datasets(data_name, dataroot, normalize=True, val_size=10000):
    ...
    if data_name == 'cifar10':
        ...
        normalization = transforms.Normalize((0.2860,), (0.3530,))
    ...
    # train_set, val_set, _ = torch.utils.data.random_split(dataset, [1000, 1000, len(dataset)-2000])  # update training size
...
def gen_classes_per_node(dataset, num_users, classes_per_user=2, high_prob=0.6, low_prob=0.4):
    ...
    for i in range(num_users):
        ...
        for _ in range(classes_per_user):
            ...
            c_idx += 1
            c_idx = c_idx % num_classes
        class_partitions['class'].append(c)
        class_partitions['prob'].append([class_dict[i]['prob'].pop() for i in c])
    return class_partitions


def gen_data_split(dataset, num_users, class_partitions, mode="train"):
    """
    ...
    # num_samples_per_label = np.zeros(num_classes, dtype=int)
    # for label in range(num_classes):
    #     num_samples_per_label[label] = num_samples[label]
    # prior = num_samples_per_label / num_samples_per_label.sum()
    # label_alpha = 1
    # label_dist_matrix = dir_label_matrix(num_users, label_alpha, prior)
    # label_dist_matrix = label_dist_matrix / label_dist_matrix.sum(axis=0)
    # print(label_dist_matrix)
    ...
    for usr_i in range(num_users):
        for c, p in zip(class_partitions['class'][usr_i], class_partitions['prob'][usr_i]):
            if mode == "train":
                # lt_p = 1. / (np.exp(usr_i*0.3+0.8))
                # end_idx = int(num_samples[c] * lt_p)
                end_idx = 10
            else:
                end_idx = int(num_samples[c] * p)
            user_data_idx[usr_i].extend(data_class_idx[c][:end_idx])
            data_class_idx[c] = data_class_idx[c][end_idx:]
        print(usr_i, len(user_data_idx[usr_i]), mode, class_partitions['class'][usr_i])
        # print(len(user_data_idx[usr_i]))
...
def gen_random_loaders(data_name, data_path, num_users, bz, classes_per_user):
    ...
    for i, d in enumerate(datasets):
        ...
        if i == 0:
            ...
            mode = "train"
        usr_subset_idx = gen_data_split(d, num_users, cls_partitions, mode=mode)
